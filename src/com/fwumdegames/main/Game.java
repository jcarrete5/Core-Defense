...
public class Game extends JPanel implements MouseListener, MouseMotionListener
{
	...
	public static final int UNLIMITED = -1, EASY = 0, MEDIUM = 1, HARD = 2,
			SANDBOX = 3;
	public static int lives;
	public static int money;
	public static final int options = 1;

	private static final Color SPD_COLOR = new Color(255, 0, 0, 125); // default
																		// speed
																		// color
	public static final Color BG_COLOR = new Color(0, 0, 0, 100); // background
																	// color.
																	// (slightly
																	// transparent
																	// black)
		...
		this.addKeyListener(new KeyListener()
		{
			...
			public void keyPressed(KeyEvent e)
			{
				...
				if(e.getKeyCode() == KeyEvent.VK_ESCAPE)
					...
					if(!isPaused() && curTower == null
							&& e.getKeyCode() == KeyEvent.VK_S) // open the shop
																// screen
						s.opened = !s.opened;
			}
		...
		if(!isPaused())
		{
			...
			for (Tile[] row : grid)
				for (Tile tile : row)
					if(tile.hasTower())
					{
						tile.getTower().attack(waves.get(curWave).get(0), tile); // attack
																					// the
																					// first
																					// mob
																					// in
			...
			if(startWave)
				for (int i = 0; i < waves.get(curWave).size(); i++)
				{
					...
					if(waves.get(curWave).get(i).isOnScreen())
						...
						if(!waves.get(curWave).get(i).isAlive())
			...
			outer: for (Tile[] row : grid)
				for (Tile t : row)
					if(t.hasTower())
					{
						...
						if(b)
							break outer; // only show one upgrade screen at a
											// time and save time by breaking
											// when it displays the upgrade
											// screen
					}

			// drag tower when selected from the shop
			if(curTower != null)
				g2.drawImage(curTower.getImage(), curMousePos.x - 25,
						curMousePos.y - 25, this);
			...
			g2.drawString(
					"Lives: " + (lives == UNLIMITED ? "infinite" : lives), 5,
					10);
			g2.drawString(
					"Money: " + (money == UNLIMITED ? "infinite" : money), 5,
					25);
			...
			if(startWave)
			{
				...
				g2.fillPolygon(new int[] { 776, 776, 783 }, new int[] { 11, 24,
						17 }, 3);
				g2.fillPolygon(new int[] { 783, 783, 790 }, new int[] { 11, 24,
						17 }, 3);
			}
			else
			{
				// draw play button
				g2.fillPolygon(new int[] { 777, 777, 789 }, new int[] { 11, 24,
						17 }, 3);
			}
		}
		else
	...
	public void mouseClicked(MouseEvent e)
	{
		if(shop.opened)
		{
			...
			if(col >= 0 && row >= 0)
			{
				...
				// if you have less money than the tower is worth (unless in
				// sandbox) after clicking, then leave the shop open
				shop.opened = shop.towers[row][col] != null
						&& (money < shop.towers[row][col].getCost() && !(money == UNLIMITED)) ? true
						: false;
			}
		}
		else
			if(curTower != null)
			{
				...
				if(grid[row][col].getType() != Tile.PATH
						&& grid[row][col].getType() != Tile.FAULT)
					grid[row][col].addTower(curTower);

				curTower = null;
			}
			else
				if(e.getPoint().x >= 770 && e.getPoint().x < 795
						&& e.getPoint().y >= 5 && e.getPoint().y < 30) // play/fast-forward
																		// is
																		// pressed
				{
				...
				else
				{
					...
					// if a tower is showing an upgrade screen already it is
					// bound to 'already'
					Tower already = null;
					...
					if(grid[row][col].hasTower()) // if the clicked tile has a
													// tower
					{
						if(!grid[row][col].getTower().viewUpgradeScreen) // and
																			// the
																			// clicked
																			// tower
																			// isn't
																			...
																			// upgrade
																			// screen
							if(already == null) // and there isn't a tower
												// already showing an upgrade
												// screen, then show the upgrade
												// screen for the clicked tower
								grid[row][col].getTower().viewUpgradeScreen = true;
							else
							// if there is an upgrade screen already showing,
							// then show the upgrade screen for the clicked
							// tower and not for the one already showing
							{
						...
						if(already != null) // if another tower is showing an
											// upgrade screen and your mouse is
											// on the X, then close the update
											// screen
						{
							Point p = e.getPoint();
							if(p.x >= 675 && p.x <= 695 && p.y >= 5
									&& p.y <= 25)
								already.viewUpgradeScreen = false;
						}
				}
