...
/**
 * Levels are displayed in the order they appear in the file. This class aids in
...
public class Level
{
	...
	public Level(Element lvlNode)
	{
		...
		for (int row = 0; row < grid.length; row++)
			...
			{
				// terminate the program if there aren't enough Tile Type to
				// load the map
				if(!scanLayout.hasNext())
				{
					JOptionPane.showMessageDialog(null,
							"Tile Type deficiency in the \"levels.xml\" file at level: \""
									+ lvlNode.getAttribute("lvlName") + "\"."
									+ "\nFix this error to continue",
				...
				case Tile.FAULT:
					imgIndex = 2;
					break;
				default: // handle what happens if the level creator uses an
							// invalid Tile Type
					JOptionPane.showMessageDialog(null,
							"Invalid XML Level Element - lvlName=\"" + this
									+ "\":\nRogue Tile type of: " + type,
				...
				grid[row][col] = new Tile(
						CoreDefense.sprites[imgIndex][CoreDefense.TILE],
						col * 50, row * 50, type);
			}
	...
	{
		Element wavesNode = (Element) lvlNode.getElementsByTagName("Waves")
				.item(0);
	...
	{
		int option = JOptionPane.showOptionDialog(frame, "Play on "
				+ (d == Game.EASY ? "EASY" : d == Game.MEDIUM ? "MEDIUM"
						: d == Game.HARD ? "HARD" : "SANDBOX") + " ?",
				"Difficulty Selection", JOptionPane.YES_NO_CANCEL_OPTION,
				JOptionPane.QUESTION_MESSAGE, null, null, null);
		if(option == JOptionPane.YES_OPTION)
			return d;
		else
			if(option == JOptionPane.NO_OPTION && d != 3)
