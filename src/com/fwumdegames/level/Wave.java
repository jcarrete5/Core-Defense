...
public class Wave extends ArrayList<Mob>
{
	...
	public Wave(Element wave, int x, int y, Tile[][] grid)
	{
		...
		{
			int rank = Integer.parseInt(((Element) mobs.item(i))
					.getAttribute("rank"));
			int amount = Integer.parseInt(((Element) mobs.item(i))
					.getAttribute("amount"));
			double spawnTime = Double.parseDouble(((Element) mobs.item(i))
					.getAttribute("spawnTime"));
