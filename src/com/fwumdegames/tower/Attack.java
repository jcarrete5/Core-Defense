...
public class Attack
{
	 ...
	 * @param anim
	 * @param x
	 *            starting position of the Attack
	 * @param y
	 *            starting position of the Attack
	 * @param m
	 */
	public Attack(Image[] anim, int x, int y, Mob m)
	{
		...
		/*
		 * sqrt((x2 - x1)^2 + (y2 - y1)^2) -- distance formula m = (y2 - y1) /
		 * (x2 - x1) -- slope formula
		 */
		int yPrime = (m.getY() + 25) - (y1 + 25), xPrime = (m.getX() + 25)
				- (x1 + 25); // find the slope of the line between the mob and
								// the tower
	...
	public void draw(Graphics2D g2)
	{
		...
		r = r > anim.length - 0.8 ? 0 : r + 0.5; // controls how long each
													// rotation is visible for.
		...
		// after redrawing attack, recalculate the path of the attack to make it
		// more accurate
		int yPrime = (target.getY() + 25) - (y1 + 25), xPrime = (target.getX() + 25)
				- (x1 + 25); // find the slope of the line between the mob and
								// the tower
	...
	{
		Rectangle targetHitBox = new Rectangle(target.getX(), target.getY(),
				50, 50);
