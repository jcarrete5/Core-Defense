...
public class PickaxeTower extends Tower
{
	...
	{
		if(m.getX() <= tile.getX() + super.getRangePixels() + 49
				...
				&& m.isAlive())
		{
			...
			if((int) atk == 1)
			{
				...
				attackQueue.add(new Attack(new Image[] {
						CoreDefense.sprites[0][CoreDefense.ATTACK],
						CoreDefense.sprites[1][CoreDefense.ATTACK],
						CoreDefense.sprites[2][CoreDefense.ATTACK],
						CoreDefense.sprites[3][CoreDefense.ATTACK] }, tile
						.getX(), tile.getY(), m));
			}
			...
			for (Attack atk : attackQueue
					.toArray(new Attack[attackQueue.size()]))
				// convert to a regular array to avoid
				// ConcurrentModificationException
				if(atk.getX() + 25 < 0 || atk.getX() >= 800
						|| atk.getY() + 25 < 0 || atk.getY() >= 800)
					attackQueue.remove(atk);
				else
					if(atk.hasHitMob() && m.isAlive())
					{
						attackQueue.remove(atk);
						m.hit();
					}
